    ...
    internal class ProductRepository : IRepository<Product>
    {
        ...
        public async Task<IEnumerable<Product>> GetAllAsync()
        {
            ...
            using (var db = new MySqlConnection(_connectionString))
            {
                ...
                {
                    await db.OpenAsync();
                    var query = "SELECT * FROM termekekview";
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            {
                                Id = reader.GetInt32("TermekID"),
                                Name = reader.GetString("Marka"),
                                Brand = reader.GetInt32("Kep"),
                                Color = reader.GetString("Szín"),
                                Size = reader.GetString("Meret"),
                                Stock = reader.GetInt32("Készlet"),
                                Price = reader.GetDecimal("TermekAr")
        ...
        public async Task<Product> GetByIdAsync(string id)
        {
            ...
            using (MySqlConnection db = new MySqlConnection(_connectionString))
            {
                ...
                {
                    await db.OpenAsync();
                    string query = "SELECT * FROM termekview WHERE id = @PrimaryKeyValue;";
        ...
        public async Task DeleteAsync(string id)
        {
            using (MySqlConnection db = new MySqlConnection(_connectionString))
            {
                ...
                {
                    await db.OpenAsync();
                    string deleteQuery = "DELETE FROM termekview WHERE id = @PrimaryKeyValue;";
                    MySqlCommand deleteCmd = new MySqlCommand(deleteQuery, db);
                    deleteCmd.Parameters.AddWithValue("@PrimaryKeyValue", id);
                    await deleteCmd.ExecuteNonQueryAsync();
                    MessageBox.Show($"Adat törölve a(z) termek táblából, azonosító: {id}");
                }
                catch (Exception ex)
                {
                    MessageBox.Show("Adatbázis kapcsolati hiba!", $"Hiba! {ex.Message}", MessageBoxButton.OK, MessageBoxImage.Error);
        ...
        public async Task CreateAsync(Dictionary<string, object> details)
        {
            using (var db = new MySqlConnection(_connectionString))
            {
                try
                {
                    ...
                    var query = $"INSERT INTO termekview ({columns}) VALUES ({values})";
                    ...
                    MessageBox.Show($"Termek hozzaadva!");
                }
                catch (Exception ex) {
                    MessageBox.Show("Adatbázis kapcsolati hiba!", $"Hiba! {ex.Message}", MessageBoxButton.OK, MessageBoxImage.Error);
                }
        ...
        public async Task UpdateAsync(string id, Dictionary<string, object> details)
        {
            using (var db = new MySqlConnection(_connectionString))
            {
                try
                {
                    ...
                    var query = $"UPDATE termekview SET {updates} WHERE id = @id";
                    ...
                    await cmd.ExecuteNonQueryAsync();
                    MessageBox.Show($"Termek adatai frissitve! Azonosito: {id}");
                }
                catch (Exception ex)
                {
                    MessageBox.Show("Adatbázis kapcsolati hiba!", $"Hiba! {ex.Message}", MessageBoxButton.OK, MessageBoxImage.Error);
