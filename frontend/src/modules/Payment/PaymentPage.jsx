                    ...
                    <div className="lg:col-span-2 max-lg:order-1 p-6 !pr-0 max-w-4xl mx-auto w-full">
                        ...
                        <form className="lg:mt-16">
                            ...
                            <div className="flex flex-wrap gap-4 mt-8">
                                <button type="button" className="min-w-[150px] px-6 py-3.5 text-sm bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300" onClick={() => navigate(-1)}>Back</button>
                                <button type="button" className="min-w-[150px] px-6 py-3.5 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700">Confirm payment ${finalTotal.toFixed(2)}</button>
                            </div>
                        </form>
                    </div>
                    ...
                    <div className="bg-gray-100 lg:h-screen lg:sticky lg:top-0 lg:max-w-[430px] w-full lg:ml-auto">
                        <div className="relative h-full">
                                        ...
                                        <div key={`${item.id}-${item.Szín}-${item.Meret}`} className="flex gap-4">
                                            ...
                                            <div className="w-full">
                                                ...
                                                <ul className="text-xs text-gray-800 space-y-1 mt-2">
                                                    <li className="flex flex-wrap gap-4">Size <span className="ml-auto">{item.Meret}</span></li>
                                                    <li className="flex flex-wrap gap-4">Quantity <span className="ml-auto">{item.quantity}</span></li>
                                                    <li className="flex flex-wrap gap-4">Total Price <span className="ml-auto">{(item.TermekAr * item.quantity).toFixed(2)} Ft</span></li>
                                                </ul>
                                            </div>
                                        </div>
                                    ))}
                            ...
                            <div className="lg:absolute lg:left-0 lg:bottom-0 bg-gray-200 w-full p-4">
                                <h4 className="flex flex-wrap gap-4 text-sm text-gray-800 font-bold">Total <span className="ml-auto">{finalTotal.toFixed(2)} Ft</span></h4>
                            </div>
                        </div>
                    </div>
                </div>
